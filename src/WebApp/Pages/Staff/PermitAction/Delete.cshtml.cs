    ...
    IAuthorizationService authorization) : PageModel
{
    ...
    public async Task<IActionResult> OnGetAsync(Guid? actionId)
    {
        ...
        if (actionItem.IsDeleted)
        {
            TempData.SetDisplayMessage(DisplayMessage.AlertContext.Warning,
                "Permit Action cannot be deleted because it is already deleted.");
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        await actionService.DeleteAsync(PermitActionItemId);
        HighlightId = PermitActionItemId;
        TempData.SetDisplayMessage(DisplayMessage.AlertContext.Success, "Permit Action successfully deleted.");
